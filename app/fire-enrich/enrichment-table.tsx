...
export function EnrichmentTable({ rows, fields, emailColumn }: EnrichmentTableProps) {
  ...
  const getCellAnimationDelay = useCallback((rowIndex: number, fieldIndex: number) => {
    ...
    const totalRowAnimationTime = 2000;
    const delayPerCell = Math.min(300, totalRowAnimationTime / fields.length);
  ...
  const startEnrichment = useCallback(async () => {
    ...
    try {
      // =================================================================
      // THE FIX IS HERE: Read ALL three keys from localStorage
      // =================================================================
      const firecrawlApiKey = localStorage.getItem('firecrawl_api_key');
      const openaiApiKey = localStorage.getItem('openai_api_key');
      const apolloApiKey = localStorage.getItem('apollo_api_key'); // <-- ADDED THIS LINE

      // Check if any key is missing before making the call
      if (!firecrawlApiKey || !openaiApiKey || !apolloApiKey) {
        toast.error("API keys missing. Please refresh and provide all API keys.");
        setStatus('cancelled');
        return;
      }
      ...
      const headers: Record<string, string> = {
        ...
        'X-Apollo-API-Key': apolloApiKey, // <-- ADDED THIS LINE
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              switch (data.type) {
                ...
                case 'result':
                  setResults(prev => new Map(prev).set(data.result.rowIndex, data.result));
                  setRowDataArrivalTime(prevTime => new Map(prevTime).set(data.result.rowIndex, Date.now()));
